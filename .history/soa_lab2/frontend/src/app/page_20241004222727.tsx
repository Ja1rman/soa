...
const FlatsTable: React.FC = () => {
  ...
  const handleSave = async () => {
    if (editFormData) {
      try {
        // Создание XML строки из editFormData
        const xmlData = `
          <Flat>
            <id>${editFormData.id}</id>
            ...
            <furnish>${editFormData.furnish || 'NONE'}</furnish>
            <view>${editFormData.view || 'NONE'}</view>
            <transport>${editFormData.transport}</transport>
            <house>
              <name>${editFormData.house?.name || ''}</name>
              <year>${editFormData.house?.year || 0}</year>
        ...
        const response = await fetch(`/api/flats/${editFormData.id}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/xml', // Указание типа контента XML
          },
          body: xmlData,
        });
  
        if (response.ok) {
          const updatedFlat: Flat = await response.json(); // Обновите, если API также возвращает обновлённые данные
          setFlats((prevFlats) =>

...
const FlatsTable: React.FC = () => {
  ...
  const [addFurnish, setAddFurnish] = useState<Furnish | ''>('');
  const [addView, setAddView] = useState<View | ''>('');
  const [addTransport, setAddTransport] = useState<Transport | ''>('');

  const fetchFlatsWithFurnishLessThan = async () => {
    try {
      const response = await fetch(`/flats/furnish-less-than?maxFurnish=${maxFurnish}`);
      const xmlResponse = await response.text();
  ...
  const fetchMinCoordinates = async () => {
    try {
      const response = await fetch('/flats/min-coordinates');
      const xmlResponse = await response.text();
...
`}</style>
      <form onSubmit={handleAddFlat}>
        ...
        <input
          ...
          required
        />
        <input type="text" value={houseName} onChange={(e) => setHouseName(e.target.value)} placeholder="House Name" />
        <input type="number" value={houseYear} onChange={(e) => setHouseYear(Number(e.target.value))} placeholder="House Year" />
        <input type="number" value={numberOfFloors} onChange={(e) => setNumberOfFloors(Number(e.target.value))} placeholder="Number of Floors" />
        <input type="number" value={numberOfLifts} onChange={(e) => setNumberOfLifts(Number(e.target.value))} placeholder="Number of Lifts" />
      ...
      <div>
        <button onClick={fetchMinCoordinates}>Get Flat with Min Coordinates</button>

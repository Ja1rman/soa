...
const fetchFlats = async (page: number, sortFields: string): Promise<Flat[]> => {
  const response = await fetch(API_URL + `/flat-management/flats?page=${page}&sort=${sortFields}`, {
    headers: {
...
const FlatsTable: React.FC = () => {
  ...
  const [sortFieldsField, setSortFieldsField] = use
  ...
  useEffect(() => {
    const loadFlats = async () => {
      try {
        const fetchedFlats = await fetchFlats(currentPage, sortFields);
  ...
  const resetToDefaultTable = async () => {
    try {
      const fetchedFlats = await fetchFlats(currentPage, 'id');
      setFlats(fetchedFlats);
  ...
  return (
    <div>
      ...
      <div className="base-element">
        <input
          type="text"
          onChange={(e) => setSortFieldsConst(e.target.value)}
          placeholder="Введите поля для сортировки"
        />
      ...
      <div className="base-element">
        <input
          ...
          placeholder="Введите минимальное число комнат"
        />

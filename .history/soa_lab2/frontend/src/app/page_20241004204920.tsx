...
const fetchFlats = async (): Promise<Flat[]> => {
  ...
  return data.map((flatData: { id: number; name: string; coordinates: { x: number; y: number; }; area: number; numberOfRooms: number; transport: Transport; creationDate: string | number | Date; furnish: Furnish; view: View; house: { name: number; year: number; numberOfFloors: string | undefined; numberOfLifts: number | undefined; }; }) => new Flat(
    flatData.id,
    flatData.name,

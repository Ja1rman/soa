...
const FlatsTable: React.FC = () => {
  ...
  const [furnish, setFurnish] = useState<Furnish | undefined>(undefined);
  const [view, setView] = useState<View | undefined>(undefined);
  const [transport, setTransport] = useState<Transport | Transport.NONE>(Transport.NONE);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const newFlat: Flat = {
      ...
      creationDate: new Date(), // буедт присвоен на сервере
      area: Number(area),
    ...
    // Вызов функции onAdd для добавления нового Flat
    onAdd(newFlat);
    ...
    setArea(0);
    setNumberOfRooms(0);
    setFurnish(undefined);
    setView(undefined);
    setTransport(Transport.NONE);
    setHouseName('');
    setHouseYear(0);
    setNumberOfFloors(0);
    setNumberOfLifts(0);

...
const FlatsTable: React.FC = () => {
  ...
  const [minRooms, setMinRooms] = useState<number>(1); // начальное значение для minRooms
  const [roomCount, setRoomCount] = useState<number | null>(null);

  const handleFetchRoomCount = async () => {
    try {
      const response = await fetch(API_URL + `/flats/number-of-rooms?minRooms=${minRooms}`);
      if (!response.ok) {
        throw new Error('Ошибка при получении данных');
      }
      const text = await response.text();
      setRoomCount(parseInt(text, 10)); // обновляем количество комнат в состоянии
    } catch (err) {
      alert(err);
    }
  };

...
const FlatsTable: React.FC = () => {
  ...
  const handleFilter = async (cheapest: string, withBalcony: string) => {
    try {
      const response = await fetch(`/find-with-balcony/${cheapest}/${withBalcony}`);
      const xmlResponse = await response.text();
      
      const parsedFlats = await parseFlatsFromXml(xmlResponse);  // Парсинг XML ответа в массив квартир
      setFilteredFlats(parsedFlats);
    } catch (err) {
      console.error('Error fetching filtered flats:', err);
    }
  };
